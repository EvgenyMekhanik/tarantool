...
static int
test_mp_compress_small(enum compression_type compression_type, uint32_t level)
{
	plan(2);
...
static int
test_mp_compress_large(enum compression_type compression_type, uint32_t level)
{
	plan(2);
...
test_mp_decompress_errors(void)
{
	...
	is(mp_decompress(&d, ddata, 1), 0, "mp_decompress");

	cdata_end[-2] = 17;
	d = cdata;
	/* Corrupt data */
	is(mp_decompress(&d, ddata, sizeof(ddata)), 0, "mp_decompress");
...
main(void)
{
	plan((compression_type_MAX - COMPRESSION_TYPE_NONE - 1) * 2 + 2);

	test_mp_compress_small(COMPRESSION_TYPE_ZSTD, ZSTD_maxCLevel());
	test_mp_compress_large(COMPRESSION_TYPE_ZSTD, ZSTD_maxCLevel());
	test_mp_compress_small(COMPRESSION_TYPE_LZ4, 0);
	test_mp_compress_large(COMPRESSION_TYPE_LZ4, 0);

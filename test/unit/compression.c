...
mp_compression_test(enum compression_type compression_type)
{
	...
	for (enum mp_type type = MP_NIL; type <= MP_EXT; type++) {
		...
		rc = tt_compression_init_for_compress(&ttc, compression_type,
						      data_end - data, data);
		is(rc, 0, "init compression structure");
		uint32_t size = mp_sizeof_compression_max(&ttc);
		...
		is(mp_decode_compression((const char **)&cdata_end, &ttc),
		   0, "decompression");
	...
	rc = tt_compression_init_for_compress(&ttc, compression_type_MAX,
					      0, data);
	isnt(rc, 0, "init compression structure fail");

	/* Data for compression should be a single msgpack field. */
	data_end = data;
	data_end = mp_encode_uint(data_end, 1);
	data_end = mp_encode_uint(data_end, 2);
	rc = tt_compression_init_for_compress(&ttc, COMPRESSION_TYPE_NONE,
					      data_end - data, data);
	isnt(rc, 0, "init compression structure fail");

	/* Too big size of data for compression */
	data_end = data;
	data_end = mp_encode_uint(data_end, 1);
	rc = tt_compression_init_for_compress(&ttc, COMPRESSION_TYPE_NONE,
					      data_end - data, data);
	is(rc, 0, "init compression structure");
	/* Change size manually for test purpose. */
	ttc.size = UINT32_MAX;
	data_end = mp_encode_compression(data, &ttc);
	is(data_end, NULL, "compression fail");
...
main(void)
{
	plan(compression_type_MAX - COMPRESSION_TYPE_NONE);
	memory_init();
        fiber_init(fiber_c_invoke);
        random_init();
        msgpack_init();

        for (enum compression_type type = COMPRESSION_TYPE_NONE;

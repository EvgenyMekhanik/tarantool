...
/**
 * Same as mp_decompress, but shpuld be called after
 * decoding ext header.
 */
static size_t
	    ...
	    enum compression_type type, uint32_t level)
{
	...
	if (mp_sizeof_ext(size + extra_size) + extra_size >= src_size)
		goto no_compression;
...
mp_snprint_compression(char *buf, int size, const char **data, uint32_t len)
{
	...
	if (mp_decompress_raw(data, len, svp, dst, dst_size) != dst_size) {
		*data = svp;
		free(dst);
		return -1;
	}

...
compress_tuple_new(struct space *space, const char *data,
		   const char *data_end)
{
	if (space_is_system(space) || !space->format->is_compressed) {
	...
	for (uint32_t i = 0; i < field_count && i < array_size; i++) {
		...
		if (tt_compression_init_for_compress(&ttc, type,
						     d - dp, dp) != 0)
			goto finish;
		total_size += mp_sizeof_compression_max(&ttc);
	}
	...
	for (uint32_t i = 0; i < field_count && i < array_size; i++) {
		...
		if (tt_compression_init_for_compress(&ttc, type,
						     d - dp, dp) != 0)
			goto finish;
		tmp_data_end = mp_encode_compression(tmp_data_end, &ttc);
		if (tmp_data_end == NULL)
			goto finish;
	}
	assert(d <= data_end);
	memcpy(tmp_data_end, d, data_end - d);
	tmp_data_end += data_end - d;
	new_tuple = space->format->vtab.tuple_new(space->format, tmp_data,
						  tmp_data_end);
finish:
...
{
	 if (space_is_system(space) || !old_tuple->is_compressed) {
		uint32_t bsize;
